...
DatabaseHelpers.recalcBalance = (clientID) => __awaiter(_a, void 0, void 0, function* () {
    let events;
    try {
        events = yield Event.find({ clientID: clientID });
    }
    catch (err) {
    ...
    try {
        yield Client.findOneAndUpdate({ _id: clientID }, { balance: clientBalance }, { upsert: true });
        yield Event.bulkWrite(updatedEventsBulkWrite);
...
DatabaseHelpers.getAllData = (query) => __awaiter(_a, void 0, void 0, function* () {
...
DatabaseHelpers.deleteOldEvents = (clientID) => __awaiter(_a, void 0, void 0, function* () {
    try {
        const events = yield Event.find({ clientID: clientID });
        yield Client.findOneAndUpdate({ _id: clientID }, { sessions: events });
        return Promise.resolve();
    }
    catch (err) {
        return Promise.reject(err);
