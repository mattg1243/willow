...
const addEvent = (req, res) => __awaiter(this, void 0, void 0, function* () {
    /* DEBUG LOGS
    ...
    */
    ...
    try {
        ...
        yield Client.findOneAndUpdate({ _id: req.body.clientID }, { $push: { sessions: event } });
        // get events to recalculate the running balances for each event
        yield DatabaseHelpers.recalcBalance(req.body.clientID);
...
const updateEvent = (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    */
    ...
    detail = req.body.detail;
    try {
        yield Event.findOneAndUpdate({ _id: req.params.eventid }, { type: req.body.type, duration: duration, rate: rate, amount: amount, detail: detail });
        yield DatabaseHelpers.recalcBalance(req.body.clientID);
        const response = yield DatabaseHelpers.getAllData({ _id: req.body.user });
        return res.status(200).json(response);
    }
...
const deleteEvent = (req, res) => {
    ...
    try {
        ...
        Event.findByIdAndDelete(req.body.eventID, (err, event) => __awaiter(this, void 0, void 0, function* () {
            if (err)
                throw err;
            // update clients balance and remove event from client document
            yield Client.findOneAndUpdate({ _id: req.body.clientID }, { $pull: { sessions: { _id: event._id } } });
            yield DatabaseHelpers.recalcBalance(req.body.clientID);
            yield DatabaseHelpers.deleteOldEvents(req.body.clientID);
            const response = yield DatabaseHelpers.getAllData({ _id: req.body.user });
            return res.status(200).json(response);
        }));
    }
...
const makeStatement = (req, res) => {
    ...
    async.parallel([
        ...
        exec(`moxie/target/release/moxie '${JSON.stringify(clientInfo)}' '${JSON.stringify(eventsList)}' '${JSON.stringify(providerInfo)}'`, { shell: true }, (error, stdout, stderr) => {
            if (error) {
...
const downloadStatement = (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
