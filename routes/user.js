...
router.post('/newclient', 
  validators.newClientValidator,
  validators.validate,
  helpers.verifyJWT, 
  (req, res) => { handlers.addNewClient(req, res) }
);
// delete a client and all associated events
router.post('/deleteclient', 
  validators.deleteClientValidator, 
  validators.validate, 
  helpers.verifyJWT, 
  (req, res) => { handlers.deleteClient(req, res) }
);
// update a clients information
router.post('/updateclient', 
  validators.udpateClientValidator,
  validators.validate,
  helpers.verifyJWT, 
  (req, res) => { handlers.updateClientInfo(req, res) }
);
// reset password
router.post('/resetpassword', 
  validators.resetPasswordValidator,
  validators.validate,
  (req, res) => { handlers.resetPassword(req, res) }
);
// save new password
