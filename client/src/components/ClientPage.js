...
function ClientPage(props) {
  ...
  const getClient = async () => {
    const res = await axios.get(`/client?id=${id}`, { headers: { Authorization: `Bearer ${token}` } });
    console.log(res);
  };
      ...
      <VStack style={{ height: '100%', width: '100%', paddingTop: '1rem' }}>
        <HStack spacing={10} style={{ paddingRight: '2rem', paddingLeft: '2rem' }}>
          <VStack style={{ display: 'flex', flexDirection: 'column', alignItems: 'start' }}>
            <Heading style={{ fontFamily: '"Quicksand", sans-serif', fontSize: '3rem' }}>
          ...
          <VStack
            style={{
              flexDirection: breakpoints[currentBreakpoint] > breakpoints.tablet ? 'row' : 'column',
              alignItems: 'end',
              justifyContent: 'end',
            }}
          >
        ...
        <Table
          ...
          style={{
            marginBottom: '2rem',
            width: breakpoints[currentBreakpoint] < breakpoints.tablet ? '100%' : '70%',
            padding: '1.5rem',
            tableLayout: 'fixed',
          }}
        >
          <Thead width="100%">
            <Tr style={{ width: '100%', marginLeft: 'auto', marginRight: 'auto', textAlign: 'justify' }}>
              ...
              <Th style={{ textAlign: 'center' }}>
                {breakpoints[currentBreakpoint] < breakpoints.desktop ? '$' : 'Amount'}
              </Th>
                ...
                <Tr key={event._id}>
                  ...
                  {breakpoints[currentBreakpoint] > breakpoints.tablet ? (
                    <>
                      ...
                      >
                        <Tooltip label={event.detail}>{event.detail ? event.detail : '-'}</Tooltip>
                      </Td>
        ...
        <Modal
          motionPreset="slideInBottom"
          onClose={() => {
            setEditEventIsShown(false);
          }}
          isOpen={editEventIsShown}
        >
          <ModalOverlay />
          <ModalContent pb={5}>
            ...
            <ModalBody>
              <AddEventForm
                id={id}
                event={toEdit}
                rate={client.rate ? client.rate : 'Not specified'}
                edit={true}
                setIsShown={setEditEventIsShown}
              />
            </ModalBody>
          </ModalContent>
        </Modal>
        <QuickStatement
          statementDrawerOpen={statementDrawerOpen}
          setStatementDrawerOpen={setStatementDrawerOpen}
          client={client}
        />
      </VStack>
    </>
