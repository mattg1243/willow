...
import { Input, VStack, HStack, Button, Divider, Select, FormLabel } from '@chakra-ui/react';
...
export default function AddEventForm(props) {
  ...
  const [date, setDate] = useState(
    props.event ? moment.utc(props.event.date).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD')
  );
  ...
  const [minutes, setMinutes] = useState(
    ...
    props.event ? (props.event.duration % 1 != 0 ? '0.' + (props.event.duration + '').split('.')[1] : '0.0') : 0
  );
      ...
      <VStack justifyContent="center">
        ...
        <Select
          onChange={(e) => {
            ...
            if (e.target.value === 'Refund' || e.target.value === 'Retainer' || e.target.value === 'Payment') {
              setHrlyEvent(false);
            }
          }}
          value={type}
          ...
          <VStack>
            ...
            <HStack spacing={10}>
              ...
              <Select
                ...
                defaultValue={isNaN(parseFloat(minutes)) ? '0.0' : minutes}
              >
        ...
        <Input
          type="number"
          value={rate}
          style={{ display: !hrlyEvents.includes(type) ? 'none' : 'flex' }}
          onChange={(e) => {
        ...
        <Input
          type="number"
          value={amount}
          style={{ display: !hrlyEvents.includes(type) ? 'flex' : 'none' }}
          onChange={(e) => {
